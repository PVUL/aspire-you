...
export default async function checkRepo(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      await octokit.rest.repos.get({ owner: user.login, repo: slug });
      // If it succeeds, the repo already exists natively on github!
    ...
    console.error("Check Repo API Error:", error);
